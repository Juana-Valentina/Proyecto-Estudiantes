...
export class RegisterComponent implements OnInit {
  ...
  constructor( 
    ...
    form = new FormGroup({
      name: new FormControl('', [Validators["required"], Validators["pattern"](Expresionesr.nameu)]), 
    ...
    getUsers(): void {
      ...
      this.api.getObs(apiRouters.USERS, true).subscribe({ 
        next: (resp) => {
          ...
          let usuario = data.find(item => item.email === this.form.value.email);  // Corrección aquí
          console.log(usuario);
